...
void main() {
  test('sales and purchases parse their distinct API total/count fields', () {
    final sales = SummaryReportModel.fromSalesJson({
      'netSales': 450, 'vatAmount': 58.5, 'totalSalesCount': 3,
      'paymentTypeTotals': [{'paymentMethod': 'CREDIT', 'netTotal': 450}],
    });
    final purchases = SummaryReportModel.fromPurchasesJson({
      'netPurchases': 230.5, 'vatAmount': 30, 'totalPurchasesCount': 2,
      'paymentTypeTotals': [],
    });
  ...
  for (final width in [320.0, 600.0, 1200.0]) {
    testWidgets('report and stock cards fit width $width with enlarged text', (tester) async {
      tester.view.physicalSize = Size(width, 1600);
      tester.view.devicePixelRatio = 1;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
      await tester.pumpWidget(MaterialApp(home: MediaQuery(
        data: MediaQueryData(size: Size(width, 1600), textScaler: TextScaler.linear(1.3)),
        child: Scaffold(body: SingleChildScrollView(child: Column(children: [
          DashboardReportCard(title: 'Sales', period: 'This month',
            report: SummaryReportModel.fromSalesJson({
              'netSales': 1000000.25, 'vatAmount': 130000, 'totalSalesCount': 3,
              'paymentTypeTotals': [{'paymentMethod': 'BANK_TRANSFER', 'netTotal': 1000000.25}],
            }), icon: Icons.receipt_long, color: Colors.amber),
          const DashboardStockCard(overview: StockOverviewModel(trackedProducts: 40, needingAttention: 7)),
        ]))),
      )));
  ...
  testWidgets('failed section offers retry without showing a zero report', (tester) async {
    var retried = false;
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: DashboardSection<int>(
      title: 'Sales', value: AsyncError(Exception('offline'), StackTrace.empty),
      onRetry: () => retried = true, builder: (value) => Text('Total: $value'),
    ))));
