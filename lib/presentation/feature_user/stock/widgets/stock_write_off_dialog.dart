...
/// Writes stock off as expired, damaged or lost — always a decrease.
class StockWriteOffDialog extends ConsumerStatefulWidget {
  const StockWriteOffDialog({
    super.key,
  ...
  @override
  ConsumerState<StockWriteOffDialog> createState() =>
      _StockWriteOffDialogState();
}

class _StockWriteOffDialogState extends ConsumerState<StockWriteOffDialog> {
...
/// Opens [StockWriteOffDialog] as a centered dialog on wide screens, or a
/// bottom sheet on phone. Resolves `true` once the write-off is recorded.
Future<bool?> showStockWriteOffDialog(
  BuildContext context, {
  required int productId,
  required List<InventoryUnitModel> unitOptions,
}) {
  final isWide = MediaQuery.sizeOf(context).width >= AppBreakpoints.tablet;
  final content = StockWriteOffDialog(
    productId: productId,
